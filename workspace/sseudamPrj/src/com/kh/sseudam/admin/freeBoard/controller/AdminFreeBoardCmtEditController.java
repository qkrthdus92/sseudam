...
public class AdminFreeBoardCmtEditController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(freeBoardCmtList != null) {
			...
			req.getRequestDispatcher("/views/admin/freeBoard/editCmt.jsp").forward(req, resp);
		}else {
			req.setAttribute("msg", "관리자페이지 자유게시판 댓글 수정단계 진행 중 에러발생");
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//글번호랑 댓글번호로 디비에 업데이트
		int result = new AdminFreeBoardService().editFreeBoardCmt(vo);
