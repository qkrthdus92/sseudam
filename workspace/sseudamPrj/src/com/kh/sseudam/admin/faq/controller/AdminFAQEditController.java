package com.kh.sseudam.admin.faq.controller;

import java.io.IOException;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;

import com.kh.sseudam.admin.common.AdminVo;
import com.kh.sseudam.admin.faq.service.AdminFAQService;
import com.kh.sseudam.faq.vo.FAQVo;

@WebServlet(urlPatterns = "/admin/faq/edit")
public class AdminFAQEditController extends HttpServlet{

    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {

        String no = req.getParameter("no");
        
        FAQVo vo = new AdminFAQService().FAQDetailSelect(no);
        
        req.setAttribute("faqDetailVo", vo);
        req.getRequestDispatcher("/views/admin/faq/edit.jsp").forward(req, resp);
    
    }
    
    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        
        // 세션 가져오기
        HttpSession s = req.getSession();

        // 로그인 멤버 가져오기
        AdminVo loginAdmin = (AdminVo)s.getAttribute("loginAdmin");
        
        //인코딩
        req.setCharacterEncoding("UTF-8");   
        
        //루트패스
        String root = req.getContextPath();
        
        //데이터 꺼내기
        String no = req.getParameter("no");
        String title = req.getParameter("title");
        String content = req.getParameter("content");
        String writeDate = req.getParameter("writeDate");
        String deleteYn = req.getParameter("deleteYn");
        
        String writerNo = loginAdmin.getNo();
        
        //데이터 뭉치기
        FAQVo vo = new FAQVo();
        vo.setNo(no);
        vo.setTitle(title);
        vo.setContent(content);
        vo.setWriterNo(writerNo);
        vo.setWriteDate(writeDate);
        vo.setDeleteYn(deleteYn);
        
        int result = new AdminFAQService().EditFAQOne(vo);
        
        if(result == 1) {
            //성공        
            s.setAttribute("alertMsg", "컨텐츠 수정 성공");
            s.setAttribute("loginAdmin", loginAdmin);
            resp.sendRedirect(root + "/admin/faq?pno=1");
        }else {
            //실패
            req.setAttribute("msg", "컨텐츠 수정 실패");
            req.getRequestDispatcher("/views/common/errorPage.jsp");
        }
        
    }
}
