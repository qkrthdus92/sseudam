...
public class FileUploader {
	...
	public static ReviewBoardImgVo uploadFile(Part f, String rootPath) throws IOException {
		//0. 준비
		String originName = f.getSubmittedFileName();
		String ext = originName.substring(originName.lastIndexOf("."), originName.length());
		String changeName =System.currentTimeMillis() + "_" + (Math.random()*9999 + 1) + ext;

		//1. 파일 객체 준비 (경로 + 파일명) WebContent/upload/img
		String filePath = "upload/img";
		String path = rootPath + filePath + "/";
		File target = new File(path + changeName); // "경로/파일명"
				
		//2. 데이터 넣기 (클라파일 > 자바 > 타켓파일 )
		BufferedInputStream bis = new BufferedInputStream(f.getInputStream());
		BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(target));
		
		byte[] buf = new byte[1024];
		
		//첨부된 파일을 끝까지 읽어들이고, 다시 내보내는 반복문
		int size = 0;
		while((size = bis.read(buf)) != -1) {
			bos.write(buf, 0, size); 
		}
