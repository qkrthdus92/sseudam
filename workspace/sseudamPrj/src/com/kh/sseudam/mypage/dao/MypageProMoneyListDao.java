...
public class MypageProMoneyListDao {
	...
	public static List<MypageProMoneyListVo> paySum(Connection conn, String num) {
		...
		String sql="SELECT * FROM (SELECT PA.NO ,PA.MEMBER_NO ,PA.PRO_NO ,PA.ADVICE_DATE ,PA.PAY_METHOD ,SUM(PA.PAY) OVER(ORDER BY PA.MEMBER_NO)AS PAY_SUM ,PA.PAY_DATE ,PA.STAR ,M.NAME ,M.PHONE FROM PRO_APPOINT PA LEFT JOIN MEMBER M ON PA.MEMBER_NO = M.NO WHERE PRO_NO =? ORDER BY PAY_SUM DESC) WHERE ROWNUM = 1";
