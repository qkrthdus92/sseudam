...
//			pstmt.setInt(2, end);
	...
	public int cntProByVal(Connection conn, String[] counselTypeArr, String[] proLevel, String g,
		...
		try {
			...
			while(rs.next()) {
				...
				if(proLevel != null && proLevel.length == 1 && proLevel[0].equals("level3") && !isLevel3) {
					System.out.println("마스터1개만포함 컨티뉴됨!");
					continue;
				}
				//전문 1개만 포함
		...
		System.out.println("총총총 전문가 수 : " + cnt);
		return cnt;
		//기본값
//		String counselTypeQuery = "'청소년상담','가족심리상담','우울증상담','취업/진로상담'";
//		String genderQuery = "'M','F'";
...
//		}
//		
//		String sql = "SELECT COUNT(*) FROM PRO_MEMBER M JOIN COUNSEL_TYPE T ON M.COUNSEL_TYPE_NO = T.NO WHERE PRO_STATUS = 'J' AND T.CLASS IN ("+counselTypeQuery+") AND M.GENDER IN ("+genderQuery+") AND M.PRICE IN ("+certificateQuery+")";
//
//		PreparedStatement pstmt = null;
//		ResultSet rs = null;
//		int cnt = 0;
...
//		return cnt;
	...
	public List<ProVo> selectListByVal(Connection conn, String[] counselTypeArr, String[] proLevel, String g,
		...
		if(proLevel != null && proLevel.length == 1 && proLevel[0].equals("level2")) {
			...
			sql = "SELECT * FROM ( SELECT ROWNUM AS RNUM, T.* FROM ( \r\n"
					+ "SELECT *\r\n"
					+ "FROM NOT_MASTER\r\n"
					+ "WHERE ((PRICE = 50000) OR ((COUNSEL_CNT >= 10) AND (AVG_STAR >= 4.5))) AND COUNSEL_TYPE IN ("+counselTypeQuery+") AND GENDER IN ("+genderQuery+") AND PRICE IN ("+certificateQuery+") ) T ) WHERE RNUM BETWEEN ? AND ?";
		}
		if(proLevel != null && proLevel.length == 1 && proLevel[0].equals("level1")) {
			...
			sql = "SELECT * FROM ( SELECT ROWNUM AS RNUM, T.* FROM ( SELECT *\r\n"
					+ "FROM LEVEL1\r\n"
					+ "WHERE COUNSEL_TYPE IN ("+counselTypeQuery+") AND GENDER IN ("+genderQuery+") AND PRICE IN ("+certificateQuery+") ) T ) WHERE RNUM BETWEEN ? AND ?";
		}
	...
	public List<ProVo> selectListByLatest(Connection conn,PageVo pv) {
		...
		String sql = "SELECT * FROM ( SELECT ROWNUM AS RNUM, T.* FROM ( SELECT M.NO, T.CLASS COUNSEL_TYPE,M.ID,M.PWD, M.NAME, M.GENDER,M.PHONE, M.EMAIL,M.EDUCATION,M.PRICE,M.PRO_STATUS,M.IMG,M.JOIN_DATE,M.MODIFY_DATE,M.INTRODUCE FROM PRO_MEMBER M JOIN COUNSEL_TYPE T ON M.COUNSEL_TYPE_NO = T.NO WHERE PRO_STATUS = 'J' ORDER BY JOIN_DATE DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
